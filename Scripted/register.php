...
class AuthController extends Controller
{
    ...
    public function create_database(Request $request) {
        $request->user()->currentAccesToken()->show();

        return response()->json(['message'=> 'Anda Berhasil menambahkan asset']);
    }  

    public function create_karya(Request $request) {
        $request->user()->currentAccesToken()->show();
        if ($request != null ) {
            alert('Anda Belum melakukan registrasi account/Login Account,Silahkan coba kembali');
        }
        else if ($re{
            return response()->json(['message' => 'Pastikan Data sudah Terisi dengan baik'])
        }
    }
}
